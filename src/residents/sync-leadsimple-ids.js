...
// Looks up each resident in LeadSimple by email (or name fallback)
// and stores their LeadSimple contact ID so the dashboard can link directly.
//
// Usage: node src/residents/sync-leadsimple-ids.js
//        node src/residents/sync-leadsimple-ids.js --dry-run
...
async function main() {
  ...
  // Load residents who don't have a LeadSimple ID yet
  const { data: residents, error } = await supabase
    .from('residents')
    .select('id, name, email, leadsimple_contact_id')
    .is('leadsimple_contact_id', null)

  if (error) { console.error('Failed to load residents:', error.message); process.exit(1) }
  console.log(`Residents without LeadSimple ID: ${residents.length}`)

  if (residents.length === 0) {
    console.log('All residents already have LeadSimple IDs. Done.')
    return
  }
  ...
  console.log('\nLeadSimple links in Resident Health dashboard will now go directly to each contact.')
}
